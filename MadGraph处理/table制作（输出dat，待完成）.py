...
for x in range(len(bins_T_nu)):
    ...
    bins_T_antideu.append(np.sqrt(bins_P_antideu[x] ** 2 + M_antideuteron ** 2) - M_antideuteron)  # 反氘动能bins
...
bins_lgx_antideu = [np.log10(t / M_DM) for t in bins_T_antideu]  # bins化为lg(x)刻度，x=T/M
...
wb = op.load_workbook(filename=r'/home/wangxiao/文档/dnde.xlsx')
ws = wb.worksheets[0]
ws.title = '方法一'
...
for lgx in bins_lgx:
    ...
    ws.cell(row=row_insert, column=ws.max_column, value=EventsNumber).alignment = alignment

    row_insert += 1

# 保存并打开
wb.save(r"/home/wangxiao/文档/dnde.xlsx")
